...
(**
Theorem Example14_B: forall (n1 m1 : nat),  (EQ_B (Bvar n1) (Bvar m1) ) ##  (EQ_B (Bvar m1) (Bvar n1)).
Proof.
intros .
assert(H : (Bvar 1) ## (if_then_else_B (Bvar 1) TRue FAlse)).
rewrite IFTF with (n:=1). reflexivity.
apply Forall_ELM_EVAL_B with (n:=1)(b:= (EQ_B (Bvar n1) (Bvar m1))) in H .
simpl in H.
assert ( Eq: (EQ_B (Bvar n1) (Bvar n1)) ## TRue).
apply EQ_Bool with (x := (Bvar n1)) (y:= (Bvar n1)).
reflexivity. 
rewrite <- Eq in H.
assert(H2: ( if_then_else_B (EQ_B (Bvar n1) (Bvar m1)) [ (n1+m1+1):= (Bvar n1)](EQ_B (Bvar (n1+m1 +1)) (Bvar n1))
      FAlse) ## (if_then_else_B (EQ_B (Bvar n1) (Bvar m1)) [(n1+m1+1):= (Bvar m1)](EQ_B (Bvar (n1+m1+1)) (Bvar n1))
      FAlse)).
apply EQ_BRbol_bol with (n1:= n1)(n2:= m1)(n3:=(n1+m1+1))(b1:= (EQ_B (Bvar (n1+m1+1)) (Bvar n1))) (b2:= FAlse).
simpl in H2.
assert(H4: n1<> n1+m1+1).
omega.
assert (H3: beq_nat n1 (n1+m1+1) = false).
...
rewrite IFTF with (n:=1). reflexivity.
apply Forall_ELM_EVAL_B2 with (n:=1)(b:= (EQ_B (Bvar m1) (Bvar n1))) in H5 .
simpl in H5.
rewrite H5 in H.
assert(H6:(if_then_else_B (Bvar (n1+m1+1)) (if_then_else_B (Bvar n1 ) TRue FAlse) FAlse) ## (if_then_else_B (Bvar n1)(if_then_else_B (Bvar (n1+m1+1)) TRue FAlse)(if_then_else_B (Bvar (n1+m1+1)) FAlse FAlse))).
pose proof(IFMORPH_B).
rewrite <- IFMORPH_B1 with (n2 := n1+m1+1)(b:=TRue ) (b1:=FAlse)(b2:= FAlse).
reflexivity.
apply Forall_ELM_EVAL_B with (n:=n1+m1+1)(b:= (EQ_B (Bvar n1) (Bvar m1))) in H6 .
simpl in H6. rewrite H3 in H6.
apply Forall_ELM_EVAL_B with (n:=n1)(b:= (EQ_M (Mvar m1) (Mvar n1))) in H6 .
simpl in H6.
rewrite H3 in H6.
rewrite <- beq_nat_refl in H6.
assert (H7: beq_nat (n1 + m1 + 1) n1 = false). omega.
...
assert(H8:  (if_then_else_B (EQ_M (Mvar m1) (Mvar n1)) [[ (n1+m1+1):= (Mvar m1)]](EQ_M (Mvar (n1+m1 +1)) (Mvar m1))
      FAlse) ##  (if_then_else_B (EQ_M (Mvar m1) (Mvar n1)) [[ (n1+m1+1):= (Mvar n1)]](EQ_M (Mvar (n1+m1+1)) (Mvar m1))
      FAlse)).
apply EQ_BRANCH_M1 with (n1:= m1)(n2:= n1)(n3:=(n1+m1+1))(b1:= (EQ_M (Mvar (n1+m1+1)) (Mvar m1))) (b2:= FAlse).
simpl in H8.
assert(H9: m1<> n1+m1+1).
omega.
assert (H10: beq_nat m1 (n1+m1+1) = false).
