...
Ltac funapptrmhyp m1 m2 H := 
  apply funapptrm with (t1 := m1) (t2 := m2) in H; simpl in H; try reflexivity.
 


 Ltac restrsublis H :=
   match goal with
     | [|- ?X ~ ?Y] => apply restr_sublis with (l1':= X) (l2':=Y) in H; simpl in H; try assumption; try reflexivity
   end.
...
Ltac funapp_EQ_M_in p3 p4 H := apply FUNCApp_eqm with  (p1:= p3 ) (p2:=p4) in H; unfold eqm_at_pos in H; simpl in H.
...
Ltac ifbr_t1  ml1 ml2 b b' x x' y y' := pose proof (IFBRANCH_M1 _ ml1 ml2 b b' x x' y y');
 match goal with
| [H : _ |-  (Cons _ _ (msg (ifm b x y)) ml1 ) ~  (Cons _ _ (msg (ifm b' x' y')) ml2 ) ] =>   apply H; clear H; try reflexivity
 end.

 Ltac ifbr1 :=
 match goal with
    | [ |-  (Cons _ _ (msg (ifm ?B ?X ?Y)) ?L1 )    ~  (Cons _ _ (msg (ifm ?B' ?X' ?Y')) ?L2 )  ] => ifbr_t1 L1 L2 B B' X X' Y Y'
 end.
 
Ltac ifbr_t2  ml1 ml2 b b' x1 x1' x2 x2' y1 y1' y2 y2' := pose proof (IFBRANCH_M2 _ ml1 ml2 b b' x1 x1' x2 x2' y1 y1' y2 y2');
 match goal with
| [H : _ |-  (Cons _ _ (msg (ifm b x1 y1)) (Cons _ _ (msg (ifm b x2 y2)) ml1 )) ~ (Cons _ _ (msg (ifm b' x1' y1')) (Cons _ _ (msg (ifm b' x2' y2')) ml2 )) ] =>   apply H; clear H; try reflexivity
                           end.
Ltac ifbr2 :=  match goal with
    | [ |-  (Cons _ _ (msg (ifm ?B ?X1 ?Y1)) (Cons _ _ (msg (ifm ?B ?X2 ?Y2)) ?L1 ))    ~  (Cons _ _ (msg (ifm ?B' ?X1' ?Y1')) (Cons _ _ (msg (ifm ?B' ?X2' ?Y2')) ?L2 ))  ] => ifbr_t2 L1 L2 B B' X1 X1' X2 X2' Y1 Y1' Y2 Y2'
               end.
...
Ltac ifbr_t3  ml1 ml2 b b' x1 x1' x2 x2' x3 x3' y1 y1' y2 y2' y3 y3' := pose proof (IFBRANCH_M2 _ ml1 ml2 b b' x1 x1' x2 x2' x3 x3' y1 y1' y2 y2' y3 y3');
 match goal with
| [H : _ |-  (Cons _ _ (msg (ifm b x1 y1)) (Cons _ _ (msg (ifm b x2 y2)) (Cons _ _ (msg (ifm b x3 y3)) ml1 ))) ~ (Cons _ _ (msg (ifm b' x1' y1')) (Cons _ _ (msg (ifm b' x2' y2')) (Cons _ _ (msg (ifm b' x3' y3')) ml2 ))) ] =>   apply H; clear H; try reflexivity
                           end.

Ltac ifbr3 :=  match goal with
    | [ |-  (Cons _ _ (msg (ifm ?B ?X1 ?Y1)) (Cons _ _ (msg (ifm ?B ?X2 ?Y2)) (Cons _ _ (msg (ifm ?B ?X3 ?Y3)) ?L1 )))    ~  (Cons _ _ (msg (ifm ?B' ?X1' ?Y1')) (Cons _ _ (msg (ifm ?B' ?X2' ?Y2')) (Cons _ _ (msg (ifm ?B' ?X3' ?Y3')) ?L2 )))  ] => ifbr_t3 L1 L2 B B' X1 X1' X2 X2' X3 X3' Y1 Y1' Y2 Y2' Y3 Y3'
               end.
...
Ltac ifbr_t4  ml1 ml2 b b' x1 x1' x2 x2' x3 x3' x4 x4' y1 y1' y2 y2' y3 y3' y4 y4' := pose proof (IFBRANCH_M2 _ ml1 ml2 b b' x1 x1' x2 x2' x3 x3' x4 x4' y1 y1' y2 y2' y3 y3' y4 y4');
 match goal with
| [H : _ |-  (Cons _ _ (msg (ifm b x1 y1)) (Cons _ _ (msg (ifm b x2 y2)) (Cons _ _ (msg (ifm b x3 y3)) (Cons _ _ (msg (ifm b x4 y4)) ml1 )))) ~ (Cons _ _ (msg (ifm b' x1' y1')) (Cons _ _ (msg (ifm b' x2' y2')) (Cons _ _ (msg (ifm b' x3' y3')) (Cons _ _ (msg (ifm b' x4' y4')) ml2 )))) ] =>   apply H; clear H; try reflexivity
 end.

Ltac ifbr4 :=  match goal with
    | [ |-  (Cons _ _ (msg (ifm ?B ?X1 ?Y1)) (Cons _ _ (msg (ifm ?B ?X2 ?Y2)) (Cons _ _ (msg (ifm ?B ?X3 ?Y3)) (Cons _ _ (msg (ifm ?B ?X4 ?Y4)) ?L1 ))))    ~  (Cons _ _ (msg (ifm ?B' ?X1' ?Y1')) (Cons _ _ (msg (ifm ?B' ?X2' ?Y2')) (Cons _ _ (msg (ifm ?B' ?X3' ?Y3')) (Cons _ _ (msg (ifm ?B' ?X4' ?Y4')) ?L2 ))))  ] => ifbr_t4 L1 L2 B B' X1 X1' X2 X2' X3 X3' X4 X4' Y1 Y1' Y2 Y2' Y3 Y3' Y4 Y4'
  end.
...
Theorem IFTF:  forall (n:nat),  (ifb (Bvar n) TRue FAlse) ## (Bvar n).
Proof.
intros.
...
Theorem IFTFb : forall (b:Bool),  (ifb b TRue FAlse) ## b.
